...
// Stats Data
const stats = [
  { value: 4, label: "Hackathons Won", suffix: "+" },
...
const StatCard = ({ value, label, suffix, index }) => {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center p-6 bg-black-100 rounded-2xl min-w-[150px] cursor-pointer"
    >
      <motion.span
        className="text-[#915EFF] text-4xl sm:text-5xl font-bold"
        initial={{ scale: 0 }}
        whileInView={{ scale: 1 }}
        viewport={{ once: true }}
      ...
      <span className="text-secondary text-sm sm:text-base mt-2 text-center">{label}</span>
    </motion.div>
  );
};
